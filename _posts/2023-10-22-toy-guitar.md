...
A friend wanted to use a toy guitar as a prop for a halloween costume, and asked if I could swap out the sound effects with playing some song clips. I was happy for a project that I could fit into my limited schedule, so I gave it a go.
...
It was a thrift store find, but it was still functional. I was actually surprised that it had wire strings that could actually detect being strummed.

[<img class="center" src="{{ site.image_host }}/2023/guitar/PXL_20231001_201717186_thumb.webp" height="50%" width="50%">]({{ site.image_host }}/2023/guitar/PXL_20231001_201717186.jpg)
...
I'm going to describe this process in a pretty step by step way, but in reality I was trying to figure out the set of components I'd need as quickly as possible, so I could order them and start experimenting. The issue was, that without extensive research, it was pretty hard to confirm if a set of parts were going to actually work together without having a pretty good idea of the full system.
...
The first thing I wanted to do, was confirm my hardware would be able to do the main task of reading data from an SD card out onto the speaker. I did a bit of googling, and was kind of surprised there wasn't an Arduino library that would "just work". It seems like this is usually hardware specific enough that a general library would be pretty complicated.
...
With this figured out, I still hit some issues with the button presses. These buttons had a lot of "[bounce](https://people.computing.clemson.edu/~yfeaste/855Assignments/presentations/team6-button-bounce.pdf)". This was especially a problem with my method of disambiguating the buttons using the same pins based on multiple checks. I ended up adding a check to only confirm a button state after it stayed consistent over multiple checks separated by several milliseconds. 
...
To set up the songs I used [Audacity](https://www.audacityteam.org/). While my code could handle different Wav configurations, I stuck to using mono-channel data since I only had one speaker.
...
However, it seemed like a waste to use an ESP32 without adding some sort of WiFi capability. I decided to make it possible to update the firmware, and the songs on the guitar over WiFi.
